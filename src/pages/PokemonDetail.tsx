...
const PokemonDetail: React.FC = () => {
  const { trainerId, pokemonIndex } = useParams<{
    trainerId: string;
    pokemonIndex: string;
  }>();
  ...
  const loadPokemonData = async () => {
    try {
      ...
      if (
        isNaN(pokemonIdx) ||
        !foundTrainer.team ||
        pokemonIdx >= foundTrainer.team.length
      ) {
  ...
  const handleSave = async () => {
    ...
    try {
      const updatedPokemon: Pokemon = {
        ...
        talentPoints: pokemon.talentPoints || {
          hp: 0,
          attack: 0,
          defense: 0,
          speed: 0,
  ...
  const getUsedTalentPoints = (): number => {
    ...
    return (
      pokemon.talentPoints.hp +
      pokemon.talentPoints.attack +
      pokemon.talentPoints.defense +
      pokemon.talentPoints.speed
    );
  };
  ...
  const getMaxTalentPointsForStat = (
  ...
  const getAvailableAttacks = (): Attack[] => {
    ...
    let availableAttacks = basicAttacks.filter(
      (attack) => !learnedAttackIds.includes(attack.id)
    );
  ...
  const handleForgetAttack = (attackId: string) => {
    ...
    if (attack.tier === 2) {
      ...
      refund =
        ATTACK_UPGRADE_COSTS.TIER_1_TO_2 + ATTACK_UPGRADE_COSTS.TIER_2_TO_3; // Refund both upgrades
    }

    const updatedAttacks = pokemon.learnedAttacks.filter(
      (id) => id !== attackId
    );
  ...
  const getUpgradeCost = (fromTier: 1 | 2 | 3): number => {
    switch (fromTier) {
      case 1:
        return ATTACK_UPGRADE_COSTS.TIER_1_TO_2;
      case 2:
        return ATTACK_UPGRADE_COSTS.TIER_2_TO_3;
      default:
        return 0;
    }
  };
  ...
  const getDowngradeRefund = (fromTier: 1 | 2 | 3): number => {
    switch (fromTier) {
      case 2:
        return ATTACK_UPGRADE_COSTS.TIER_1_TO_2; // Refund for going from tier 2 to tier 1
      case 3:
        return ATTACK_UPGRADE_COSTS.TIER_2_TO_3; // Refund for going from tier 3 to tier 2
      default:
        return 0;
    }
  };
  ...
  const handleUpgradeAttack = (attackId: string) => {
    ...
    if (availablePoints < upgradeCost) {
      alert(
        `Nicht genügend Talent Points! Benötigt: ${upgradeCost}, Verfügbar: ${availablePoints}`
      );
      return;
    }
  ...
  const handleEvolvePokemon = async () => {
    ...
    try {
      ...
      const evolutionDetails = await pokeApiService.getPokemonDetails(
        selectedEvolution
      );
                  ...
                  {editMode ? (
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleFormChange("secondaryType", e.target.value)
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                      >
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              ...
              {editMode && (
                <div className="pt-4 border-t border-gray-200">
                  ...
                  <div className="space-y-3">
                    {(["hp", "attack", "defense", "speed"] as const).map(
                      (stat) => {
                        const statNames = {
                          hp: "HP",
                          attack: "Angriff",
                        ...
                        const maxPointsForStat =
                          getMaxTalentPointsForStat(stat);
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex gap-1">
                              <button
                                onClick={() =>
                                  handleTalentPointChange(stat, -5)
                                }
                              ...
                              <button
                                ...
                                disabled={
                                  getAvailableTalentPoints() < 5 || isMaxed
                                }
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  {(["hp", "attack", "defense", "speed"] as const).map(
                    (stat) => {
                      ...
                      const maxPointsForStat = getMaxTalentPointsForStat(stat);
                      const isMaxed =
                        talentPoints >= maxPointsForStat && baseStat > 0;
              ...
              <div className="pt-4 border-t border-gray-200">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {editMode && (
                      <button
                        onClick={() =>
                          navigate(`/attacks/${trainerId}/${pokemonIndex}`)
                        }
                        className="px-3 py-1 bg-primary-100 text-primary-700 text-sm rounded-lg hover:bg-primary-200 transition-colors"
                      >
                        Verwalten
                      </button>
                    )}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-3 text-xs text-gray-500">
                              ...
                              <span>{attack.power} Stärke</span>
                              <span>{attack.accuracy}% Genauigkeit</span>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                {/* Quick Info */}
                {editMode && getLearnedAttacks().length > 0 && (
                  <div className="mt-3 p-2 bg-blue-50 rounded-lg">
                    <p className="text-xs text-blue-700">
                      💡 Tippe auf "Verwalten" um Attacken zu lernen, upgraden
                      oder vergessen
                    </p>
                  </div>
                )}
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 m-4 max-w-md w-full">
            ...
            {getAvailableEvolutions().length > 1 ? (
              ...
              <div className="mb-4">
                <p className="text-gray-700">
                  <span className="font-medium">{pokemon?.name}</span> wird sich
                  zu{" "}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={
                  saving ||
                  (getAvailableEvolutions().length > 1 && !selectedEvolution)
                }
